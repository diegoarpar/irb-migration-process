...
public class ETLSubjectDefines implements IETL{
    ...
    public boolean StartETL() {
        ...
        List<FSubjectDef> sourceData = sourceEM.createQuery("SELECT s FROM FSubjectDef s", FSubjectDef.class).getResultList();
        ...
        users = null;
        // Transform data
        List<SubjectDefines> transformedData = eltFactoryTransformation.getTransformation("subject").TransformData(sourceData, usersMap, applicatinosMap);

        // Load data into destination
        destEM.getTransaction().begin();
        for (SubjectDefines destEntity : transformedData) {
