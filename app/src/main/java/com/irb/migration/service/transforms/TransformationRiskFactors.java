...
public class TransformationRiskFactors implements IETLTransformation<RiskFactors, FRiskFactor> {
    ...
    public List<RiskFactors> TransformData(List<FRiskFactor> origin, Map... data) {
        return origin.stream().map(source -> {
            ...
            riskFactors.SubjectRisk = helper.fromYesNoToBoolean(source.sub_risk);
            riskFactors.ExperimentalDrug = helper.fromYesNoToBoolean(source.exp_drug);
            riskFactors.MedicalProblem = helper.fromYesNoToBoolean(source.medical_prob);
            riskFactors.PhysicalDiscomfort = helper.fromYesNoToBoolean(source.sub_phys_discomfort);
            riskFactors.MentalDiscomfort = helper.fromYesNoToBoolean(source.sub_mental_discomfort);
            riskFactors.ElectricalEquipment = helper.fromYesNoToBoolean(source.elec_equip);
            riskFactors.SubjectRecord = helper.fromYesNoToBoolean(source.sub_record);
            riskFactors.SubjectCoercion = helper.fromYesNoToBoolean(source.coercion_sub);
            riskFactors.SubjectDeception = helper.fromYesNoToBoolean(source.sub_deception);
            ...
            riskFactors.IncludeSomething = false;
