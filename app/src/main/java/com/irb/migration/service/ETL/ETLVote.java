...
public class ETLVote implements IETL{
    ...
    public boolean StartETL() {
        ...
        List<FStandardDecision> sourceData = sourceEM.createQuery("SELECT s FROM FStandardDecision s", FStandardDecision.class).getResultList();
        ...
        users = null;
        // Transform data
        List<StandardVotes> transformedData = eltFactoryTransformation.getTransformation("vote").TransformData(sourceData, usersMap, applicatinosMap);

        // Load data into destination
        destEM.getTransaction().begin();
        for (StandardVotes destEntity : transformedData) {
